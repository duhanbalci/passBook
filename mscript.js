	...
	$.ajax({
		...
		success:function(e){
			if(e==1){
				...
				tr.remove();
				$('.page-navigation').remove();
				if($('#passList tr').length % 5 == 1){s = s-1;}
...
function editEntry(e,t){
	if (e.keyCode == 13) {
		var t	= t.closest('tr');
		var lbgc= rgb2hex(t.css('background-color'));
		var to	= setInterval(function(){
			t.animate({backgroundColor: "#d8d8d8"},100);
			setTimeout(function(){t.animate({backgroundColor: lbgc},100);},100);
		},200);
		...
		$.ajax({
			...
			success:function(e){
				clearInterval(to);
				t.animate({backgroundColor: "#1bd600"},100);
				t.animate({backgroundColor: lbgc},2000);
			}
		});
	}
}
